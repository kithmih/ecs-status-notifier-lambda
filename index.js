...
export const handler = async (event) => {
  ...
  if (event.detail.lastStatus !== "STOPPED") {
    console.log(
      `Skipping notification. Task status is '${event.detail.lastStatus}', not 'STOPPED'.`
    );
    return {
      statusCode: 200,
      body: JSON.stringify({
  ...
  // Check the 'stopCode' to filter benign stops
  const benignStopCodes = ["UserInitiated", "ServiceSchedulerInitiated"];
  if (benignStopCodes.includes(event.detail.stopCode)) {
    console.log(
      `Skipping notification. Stop code '${stopCode}' indicates a planned operational stop (e.g., deployment or user action).`
    );
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "Notification skipped for benign stop code.",
      }),
    };
  }
